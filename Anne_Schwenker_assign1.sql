...
select model from planes group by engines,seats desc;
SELECT 'flight', COUNT(*) FROM flights;
SELECT 'seats', COUNT(*) FROM planes;
...
select carrier, flight, distance FROM flights where distance >1000 group by carrier order by flight desc limit 5;
# What are the top five flight destinations with the shortest air time?
select distinct dest from flights where air_time is not null  order by air_time asc limit 5;
